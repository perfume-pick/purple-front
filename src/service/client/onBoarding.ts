...
const endPoint = {
  GET_PURFUME_BRANDS: "/perpicks/perfume-brands",
  SET_RATINGS_ONBOARDING: "/perpicks/ratings/onboarding",
};

async function getPurfumeBrands() {
  const response = await axios.get<never, RestResponseType<perfumeBrands>>(
    `${process.env.NEXT_PUBLIC_ENDPOINT_EXTERNAL}${endPoint.GET_PURFUME_BRANDS}`,
    {
...
export { getPurfumeBrands, setOnboardingRating };
